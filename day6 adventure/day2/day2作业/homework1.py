...
def register():
    ...
    with open('login_account.txt','a+') as file:          ###把用户信息存入文档,a+为读写追加模式
...
def shopping_cart_show(my_cart,my_goods_dict):
    ...
    time.sleep(2)
    return (goods_all_sum,goods_all_num,my_goods_dict)                      #购物车中商品总价、数量和用户商品列表#
...
######用户注册,写入login_account.txt文件里######################



ask_question = input('是否已经有账号? y|n:')                                 ###是否有账号,是则登录,不是则注册
...
while i < 3:                                                                ###只要用户登录异常不超过3次就不断循环###
    ...
    for user_line in user_list:                                        #对帐号文件进行遍历,因为只有三个用户,所以只读前3行
        ######################分别获取帐号、密码信息和当前余额##############
        ...
        if username == user and password == passwd:                         ###如用户名和密码正常匹配###
            ...
            while first_flag:
                ...
                goods_output_dict = products_page(goods_list)             ###调用商品列表展示函数输出商品信息###
                ...
                if goods_index == 'q':                                      ###如果输入为q,则退出程序###
                    ...
                    pending_order(user_list,user_shopping_cart,now_balance)  ###调用结算函数###
