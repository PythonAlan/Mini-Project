...
def goods_list_show(my_dict):
    ...
    print('商品列表 : ')
    print('=================================================================================')
    print('%-5s  %-20s  %-10s  %-10s  %s' % ('编号','商品名称','商品价格(元)','商品总数量(个)','商品剩余数量(个)'))
    for k in my_dict:
        v = my_dict[k]
        if type(v) is dict:
...
def cart_goods_modify(modify_dict,modify_goods_dict):
    ...
    while a_flag:
        ...
        if len(index) != 0:                                                ###商品编号不为空进行字符转化###
            index = input_handle(index)                                    #调用函数(自建)
...
def user_billing(billing_list,my_cart,billing_balance):
    ...
    if my_cart:                                                             ###购物车参数不为空###
        ...
        if billing_flag == 'y':                                             ###商品结算确认###
            billing_file = open('buyer_account.txt','w')                             ###打开读写帐号文件###
...
goods_list = {
             'iphone6s': {'price':6000,'num':10,'sum':10},
...
#########################用户注册###########################################
############用户注册函数#####################
ask_question = input('是否已经有账号? y|n:')
while True:
...
while i < 3:                                                                ###只要用户登录异常不超过3次就不断循环###
    ...
    user_file = open('buyer_account.txt','r')                              ###打开帐号文件###
    user_list = user_file.readlines()                                       ###用户列表信息###
    user_file.close()                                                       ###关闭帐号文件###


    for user_line in user_list[0:3]:                                             ###对帐号文件进行遍历###
 ######################分别获取帐号、密码信息和当前余额##############
