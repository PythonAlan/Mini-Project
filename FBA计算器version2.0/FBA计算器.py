...
##################################显示部件###################################################
        ...
        ###########选择fba重量段#############
        self.v1 = StringVar()
        btSelectOne = Radiobutton(window,text='1lb以内',variable = self.v1,value = '1lb',command = self.selectLevel)
        btSelectTwo = Radiobutton(window,text='2lb以内',variable = self.v1,value = '2lb',command = self.selectLevel)

        btSelectOne.grid(row = 13,column=1,sticky=E)
        btSelectTwo.grid(row = 14,column=1,sticky=E)

        #######计算和退出按钮
        btExit = Button(window,text="退出程序",command = window.quit).grid(row=14,column=2,sticky=E)
...
##选择FBA重量段函数
    def selectLevel(self):
        fbaFee = 0
        if self.v1.get() == '1lb':
            fbaFee = settings.fee_rate['fbaFee_1lb']
        elif self.v1.get() == '2lb':
            fbaFee = settings.fee_rate['fbaFee_2lb']
